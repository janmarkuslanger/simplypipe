...
class Pipe:
    ...
    def __init__(self, source: Iterable[Any], _ops: list[_Op] | None = None) -> None:
        self._source = source
    ...
    def map(self, fn: Callable[..., Any]) -> Pipe:
    ...
    def flat_map(self, fn: Callable[..., Any]) -> Pipe:
        def op(iterable: Iterable[Any], _stats: RunStats) -> Iterator[Any]:
            for item in iterable:
                yield from fn(item)
    ...
    def filter(self, fn: Callable[..., Any]) -> Pipe:
        def op(iterable: Iterable[Any], stats: RunStats) -> Iterator[Any]:
            for item in iterable:
                if fn(item):
                    yield item
                else:
    ...
    def tap(self, fn: Callable[..., Any]) -> Pipe:
        def op(iterable: Iterable[Any], _stats: RunStats) -> Iterator[Any]:
            for item in iterable:
                fn(item)
                yield item
    ...
    def run(self, sink: Callable[..., Any] | None = None) -> RunStats:
